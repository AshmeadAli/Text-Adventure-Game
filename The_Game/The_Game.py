...
tron.set_description( "A metallic planet built by Robots, all named Sophia")
...
rennie = Enemy("Rennie(aka.Sophia)", " A stupid, humanoid robot who resembles C-3PO, ( NOT R2D2 )")
...
piton.set_items( 'wicker basket','snake oil','bug spray')
...
# Remarks
remarks_list = [ "Oh Shit", "Geeze", "You will do better next time","Feeling duncey are you?",
                 "You gotta be kidding me", "Oh bleep ##**", "Screw your head back on",
                 " Oh well, you tried"]
# Rooms List
rooms_list = [dalex,piton,tron,ghost,crystal]
...
print("go where no man has gone before, fight your way across the five planets ")
print("and defeat their strange inhabitants. You are to ignore the prime directive.")
...
while Game  :		
    print("\n")
    if current_room == teleporter:
        current_room = random.choice(rooms_list)
    ...
    if inhabitant is not None:
        ...
        if (win == False):
            ...
            print(remarks +" ! You now have " + str(life) + " lives remaining")
    if life == 0:
